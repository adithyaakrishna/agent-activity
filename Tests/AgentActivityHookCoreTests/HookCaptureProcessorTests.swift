...
final class HookCaptureProcessorTests: XCTestCase {
  func testCursorPayloadKeepsMetadataAndDropsContent() throws {
    let input = Data(
      #"{"conversation_id":"c-1","workspace_roots":["/tmp/repo"],"hook_event_name":"stop","prompt":"DO_NOT_STORE","response":"DO_NOT_STORE","command":"DO_NOT_STORE","tool_input":{"command":"DO_NOT_STORE"},"tool_response":"DO_NOT_STORE","transcript":"DO_NOT_STORE"}"#
        .utf8)
  ...
  func testClaudePayloadKeepsMetadataAndDropsContent() throws {
    let input = Data(
      #"{"session_id":"s-1","cwd":"/tmp/repo","hook_event_name":"SessionEnd","transcript_path":"/Users/example/.claude/projects/work/transcript.jsonl","prompt":"DO_NOT_STORE","response":"DO_NOT_STORE","command":"DO_NOT_STORE","tool_input":{"command":"DO_NOT_STORE"},"tool_response":"DO_NOT_STORE","transcript":"DO_NOT_STORE"}"#
        .utf8)
  ...
  func testClaudePayloadOutsideClaudeProjectsIsIgnored() throws {
    let input = Data(
      #"{"session_id":"s-1","cwd":"/tmp/repo","transcript_path":"/tmp/transcript.jsonl"}"#.utf8)
  ...
  func testHeadMetadataIsAttachedOnlyForAllowedEventAndValidSHA() throws {
    let input = Data(
      #"{"conversation_id":"c-1","workspace_roots":["/tmp/repo"],"hook_event_name":"stop"}"#.utf8)
    ...
    let record = try XCTUnwrap(
      HookCaptureProcessor(git: StubGitHeadInspector(head: validHead)).makeRecord(
        input: input,
        provider: .cursor,
        capturedAt: Date(timeIntervalSince1970: 0),
        environment: [:]
    ...
    let invalidRecord = try XCTUnwrap(
      HookCaptureProcessor(
        git: StubGitHeadInspector(
          head: GitHeadMetadata(
            sha: "not-a-sha",
            committedAt: Date(timeIntervalSince1970: 60),
            additions: 4,
            deletions: 2
