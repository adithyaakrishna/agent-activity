...
final class HookCommandLineTests: XCTestCase {
  ...
  private func hookExecutable() throws -> URL {
    let standardBuildPath = repositoryRoot().appendingPathComponent(
      ".build/debug/AgentActivityHook")
    if FileManager.default.isExecutableFile(atPath: standardBuildPath.path) {
