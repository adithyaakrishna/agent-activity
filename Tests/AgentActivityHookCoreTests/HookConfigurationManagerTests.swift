...
final class HookConfigurationManagerTests: XCTestCase {
  ...
  func testInstallPreservesUnrelatedSettingsBacksUpExistingFilesAndIsIdempotent() throws {
    ...
    XCTAssertEqual(
      cursorCommandCount(named: expectedCommand(home: home, provider: "cursor"), in: cursor), 9)
    XCTAssertEqual(cursorCommandCount(named: "existing-cursor-tool", in: cursor), 1)
    XCTAssertEqual(claude["theme"] as? String, "dark")
    XCTAssertEqual(
      claudeCommandCount(named: expectedCommand(home: home, provider: "claude"), in: claude), 9)
    ...
    XCTAssertTrue(
      FileManager.default.fileExists(
        atPath: home.appendingPathComponent(".cursor/hooks.json" + backupSuffix).path))
    XCTAssertTrue(
      FileManager.default.fileExists(
        atPath: home.appendingPathComponent(".claude/settings.json" + backupSuffix).path))
      ...
      [
        ...
        "hooks": [
          "Stop": [
            ...
            [
              "hooks": [
                ["type": "command", "command": "agent_activity_hook.sh claude", "timeout": 5]
              ]
            ],
          ]
        ],
  ...
  func testInstallRejectsMalformedConfigBeforeCopyingHelperOrChangingEitherConfig() throws {
    ...
    XCTAssertThrowsError(
      try HookConfigurationManager().install(
        helperSource: helper, homeDirectory: home, timestamp: fixedDate))
  ...
  func testInstallQuotesCommandsForShellAndEveryGeneratedCommandExecutes() throws {
    ...
    for event in claudeEvents {
      ...
      XCTAssertEqual(
        try executeShellCommand(try XCTUnwrap(hook["command"] as? String)), "capture|claude",
        "Claude event \(event)")
    }
  }
  ...
  func testInstallMigratesExactPathQualifiedLegacyCommandsAndPreservesNearMatches() throws {
    ...
    let claudeLegacy =
      "'/Users/Example/Work Space/AgentActivity/script/agent_activity_hook.sh' claude"
    ...
    try writeJSONObject(
      [
        "version": 1,
        "hooks": [
          "stop": ([cursorLegacy, "agent_activity_hook.sh cursor"] + cursorNearMatches).map {
            ["command": $0]
          }
        ],
      ], to: home.appendingPathComponent(".cursor/hooks.json"))
    try writeJSONObject(
      [
        "hooks": [
          "Stop": ([claudeLegacy, "agent_activity_hook.sh claude"] + claudeNearMatches).map {
    ...
    XCTAssertEqual(
      cursorNearMatches.filter { cursorCommands(in: cursor).contains($0) }.count,
      cursorNearMatches.count)
    XCTAssertEqual(
      claudeNearMatches.filter { claudeCommands(in: claude).contains($0) }.count,
      claudeNearMatches.count)
    ...
    XCTAssertEqual(
      ...
      claudeNearMatches.count)
    XCTAssertFalse(
      cursorCommands(in: cursor).contains(expectedCommand(home: home, provider: "cursor")))
    XCTAssertFalse(
      claudeCommands(in: claude).contains(expectedCommand(home: home, provider: "claude")))
  }
  ...
  func testUninstallPreservesPreexistingEmptyContainersAndUnrelatedMetadata() throws {
    ...
    XCTAssertEqual(
      stopGroups.first?["hooks"] as? [[String: String]],
    ...
    XCTAssertEqual(
      (claudeHooks["SessionEnd"] as? [[String: Any]])?.first?["matcher"] as? String, "preexisting")
  }
  ...
  func testReinstallCanonicalizesClaudeMetadataAndCreatesUniqueImmediateBackups() throws {
    ...
    XCTAssertEqual(
      try Data(contentsOf: backupURL(for: cursorURL, ordinal: 1)), cursorBeforeSecondInstall)
    XCTAssertEqual(
      try Data(contentsOf: backupURL(for: claudeURL, ordinal: 1)), claudeBeforeSecondInstall)
  ...
  func testHelperReplacementFailureLeavesHelperAndBothConfigsUnchanged() throws {
    ...
    XCTAssertThrowsError(
      try manager.install(
        helperSource: fixture.source, homeDirectory: fixture.home, timestamp: fixedDate))
  ...
  private func fixtureHome(
    ...
    if let claudeSettings {
      ...
      try FileManager.default.createDirectory(
        at: url.deletingLastPathComponent(), withIntermediateDirectories: true)
      try JSONSerialization.data(withJSONObject: claudeSettings, options: [.sortedKeys]).write(
        to: url)
    }
    return home
  }
  ...
  private func assertFixtureUnchanged(
    _ fixture: TransactionalFixture, file: StaticString = #filePath, line: UInt = #line
  ) throws {
    XCTAssertEqual(
      try Data(contentsOf: fixture.cursorURL), fixture.cursorData, file: file, line: line)
    XCTAssertEqual(
      try Data(contentsOf: fixture.claudeURL), fixture.claudeData, file: file, line: line)
    XCTAssertEqual(
      try Data(contentsOf: fixture.helperURL), fixture.helperData, file: file, line: line)
  }
  ...
  private func cursorCommand(event: String, in object: [String: Any]) -> String? {
    ...
    return entries?.compactMap { $0["command"] as? String }.first(where: {
      $0.hasSuffix(" capture cursor")
    })
  }

  private func claudeManagedHook(event: String, in object: [String: Any], home: URL) -> [String:
    Any]?
  {
