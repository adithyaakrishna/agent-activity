...
final class HookRecordStoreTests: XCTestCase {
  func testStoreWritesOwnerOnlyJSONLine() throws {
    ...
    XCTAssertEqual(
      try String(contentsOf: file), "{\"provider\":\"cursor\",\"session_id\":\"c-1\"}\n")
  }
  ...
  func testStoreRotatesAtLimitAndKeepsOneBackup() throws {
    ...
    try store.append(
      ["session_id": "first", "provider": "cursor"], provider: .cursor, directory: directory)
    try store.append(
      ["session_id": "second", "provider": "cursor"], provider: .cursor, directory: directory)
    try store.append(
      ["session_id": "third", "provider": "cursor"], provider: .cursor, directory: directory)
    ...
    XCTAssertFalse(
      FileManager.default.fileExists(
        atPath: directory.appendingPathComponent("cursor.jsonl.2").path))
  }
  ...
  func testStoreRejectsSerializedLineLargerThanConfiguredLimit() throws {
    ...
    XCTAssertFalse(
      FileManager.default.fileExists(atPath: directory.appendingPathComponent("cursor.jsonl").path))
    XCTAssertFalse(
      FileManager.default.fileExists(
        atPath: directory.appendingPathComponent("cursor.jsonl.1").path))
  }

  private func temporaryDirectory() throws -> URL {
    let directory = FileManager.default.temporaryDirectory
      .appendingPathComponent(
        "AgentActivityHookRecordStoreTests-\(UUID().uuidString)", isDirectory: true)
