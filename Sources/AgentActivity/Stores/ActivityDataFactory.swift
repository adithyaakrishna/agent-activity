...
enum ActivityDataFactory {
    ...
    static func make(for source: AgentSource) -> ActivityDataset {
        ...
        var days = (0..<dayCount).map { index -> ActivityDay in
            ...
            return makeDay(source: source, date: date, index: index, isInRange: inRange, calendar: calendar)
        }
    ...
    private static func fitCommitTotal(days: inout [ActivityDay], target: Int, lockedKey: String?) {
    ...
    private static func seededValue(_ seed: UInt32) -> Double {
        var value = seed
        value ^= value << 13
        value ^= value >> 17
        value ^= value << 5
