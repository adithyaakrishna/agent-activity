...
enum LocalAgentActivityError: LocalizedError {
  ...
  var errorDescription: String? {
    ...
    case .historyUnavailable(let path):
...
enum LocalAgentActivityService {
  ...
  private static func loadSynchronously(
    ...
    let rangeStart = calendar.date(
      byAdding: .day, value: -364, to: calendar.startOfDay(for: endDate))!
    ...
    return ActivityHistoryBuilder.make(from: metricsByDate, endingOn: endDate)
  }
    ...
    for root in roots where FileManager.default.fileExists(atPath: root.path) {
      ...
      for case let file as URL in enumerator {
        guard file.pathExtension == "jsonl",
          ...
          !file.lastPathComponent.contains("skill-injections")
        else { continue }
  ...
  private static func parseCursorSession(
    ...
    let subagents =
      (try? FileManager.default.contentsOfDirectory(
        at: subagentDirectory, includingPropertiesForKeys: nil))?
  ...
  private static func parseClaudeSession(
    ...
    JSONLReader.read(file) { object in
      ...
      let identifier =
        (message["id"] as? String) ?? (object["uuid"] as? String) ?? UUID().uuidString
  ...
  static func commits(matching sessions: [AgentSession]) -> [GitCommit] {
    ...
    for session in sessions {
      guard let directory = session.workingDirectory,
        allowedRoots.contains(where: { directory.isDescendant(of: $0) })
      else { continue }
    ...
    for (repositoryPath, entries) in sessionsByRepository {
      ...
      guard let first = repositorySessions.map(\.start).min(),
        let last = repositorySessions.map(\.end).max()
      else { continue }
      let commits = gitLog(
      ...
      for commit in commits
      where repositorySessions.contains(where: {
  ...
  private static func gitLog(repository: URL, since: Date, until: Date) -> [GitCommit] {
    ...
    func finishCurrent() {
      if let hash = currentHash, let date = currentDate {
        commits.append(
          GitCommit(hash: hash, date: date, additions: additions, deletions: deletions))
      }
    }
