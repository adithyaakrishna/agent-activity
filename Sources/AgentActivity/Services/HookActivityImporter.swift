  ...
  static func load(
    ...
    for file in files where FileManager.default.fileExists(atPath: file.path) {
      readJSONLines(file) { record in
        ...
        var state = states[
          sessionID, default: HookSessionState(id: sessionID, timestamp: timestamp)]
        ...
        guard let hash = record["head_sha"] as? String,
          ...
          let commitDate = ISO8601Parser.date(from: committedAt)
        else { return }
    ...
    let commits = commitCandidates.compactMap { candidate -> GitCommit? in
      guard let session = states[candidate.sessionID],
        ...
        candidate.commit.date <= session.end.addingTimeInterval(1_800)
      else { return nil }
