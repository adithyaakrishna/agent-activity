...
public struct GitHeadInspector: GitHeadInspecting {
  ...
  public func repositoryRoot(for candidateDirectory: URL) -> URL? {
    guard
      let output = gitOutput(arguments: [
        "-C", candidateDirectory.path, "rev-parse", "--show-toplevel",
      ])
    else {
      return nil
    }
  ...
  public func headMetadata(at repositoryRoot: URL) -> GitHeadMetadata? {
    guard
      let log = gitOutput(arguments: ["-C", repositoryRoot.path, "log", "-1", "--format=%H%n%cI"]),
      let statistics = gitOutput(arguments: [
        "-C", repositoryRoot.path, "show", "--numstat", "--format=", "HEAD",
      ])
    else {
      return nil
    }
    ...
    let totals = statistics.split(whereSeparator: \.isNewline).reduce(
