...
public struct HookConfigurationManager {
    ...
    public func install(helperSource: URL, homeDirectory: URL, timestamp: Date) throws {
        ...
        let updatedCursor = try installingCursorHooks(in: cursor.object, at: cursorURL, helperURL: helperURL)
        let updatedClaude = try installingClaudeHooks(in: claude.object, at: claudeURL, helperURL: helperURL)
    ...
    public func uninstall(homeDirectory: URL, timestamp: Date) throws {
        ...
        let cursor = try loadConfiguration(at: cursorURL)
        let claude = try loadConfiguration(at: claudeURL)
        let updatedCursor = try uninstallingCursorHooks(in: cursor.object, at: cursorURL, helperURL: helperURL)
        let updatedClaude = try uninstallingClaudeHooks(in: claude.object, at: claudeURL, helperURL: helperURL)
...
private extension HookConfigurationManager {
    ...
    func sanitizeCursorEntries(
        ...
        for var entry in entries {
            guard isOwnedCommand(entry["command"] as? String, helperURL: helperURL, provider: provider) else {
                result.append(entry)
                continue
            }
            removedOwned = true
    ...
    func sanitizeClaudeGroups(
        ...
        for var group in groups {
            ...
            let retained = hooks.filter { hook in
                let owned = isOwnedCommand(hook["command"] as? String, helperURL: helperURL, provider: provider)
    ...
    func installedCommand(helperURL: URL, provider: String) -> String {
        "\(posixShellQuote(helperURL.path)) capture \(provider)"
    }

    func posixShellQuote(_ value: String) -> String {
        "'" + value.replacingOccurrences(of: "'", with: "'\"'\"'") + "'"
    }

    func isOwnedCommand(_ command: String?, helperURL: URL, provider: String) -> Bool {
    ...
    func prepareHelper(from source: URL, to destination: URL) throws -> PreparedFile {
    ...
    func prepareConfiguration(
        ...
        do {
            ...
            guard (try JSONSerialization.jsonObject(with: Data(contentsOf: staged))) is [String: Any] else {
    ...
    func prepareRollback(for target: URL) throws -> URL? {
    ...
    func prepareRemoval(at target: URL) throws -> PreparedRemoval? {
        guard let rollback = try prepareRollback(for: target) else { return nil }
        return PreparedRemoval(target: target, rollback: rollback)
    }

    func commit(_ files: [PreparedFile], then removal: PreparedRemoval? = nil) throws {
        let fileManager = FileManager.default
    ...
    func replaceTarget(with file: PreparedFile) throws {
        let fileManager = FileManager.default
    ...
    func restore(_ file: PreparedFile) {
        let fileManager = FileManager.default
    ...
    func restoreRemoval(_ removal: PreparedRemoval) {
        let fileManager = FileManager.default
    ...
    func cleanup(_ files: [PreparedFile]) {
        for file in files {
    ...
    func cleanup(_ removal: PreparedRemoval) {
        try? FileManager.default.removeItem(at: removal.rollback)
    }

    func temporarySibling(of target: URL, label: String) -> URL {
        target.deletingLastPathComponent()
            .appendingPathComponent(".\(target.lastPathComponent).agentactivity-\(label)-\(UUID().uuidString)")
    }

    func uniqueBackupURL(for configURL: URL, timestamp: Date) -> URL {
        let directory = configURL.deletingLastPathComponent()
        let baseName = "\(configURL.lastPathComponent).agentactivity-backup-\(backupTimestamp(timestamp))"
